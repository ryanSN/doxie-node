...
class doxieNode {
  ...
  async get(url, responseType = 'json') {
    return this.getInitializedApi().get(url, { responseType: responseType });
  }
  ...
  async get_thumbnail(scanPath, retries = 3, retryDelay = 1000) {
    ...
    //const delays = Array.apply(null, {length: retries}).fill(retryDelay);
    //return retry(retries, this.post.bind(this), retryDelay, {url: url});
  }
