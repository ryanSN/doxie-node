...
const _handleError = response => {
  ...
  //   return new Promise(reject => reject(response.statusText));
  // }
...
class doxieNode {
  ...
  async handleRequest(url, options) {
    return await axios(`${this.getBaseURL()}${url}`, options)
      .then(_handleError)
      .then(response => response)
      .catch(error => {
        if (error.response) {
          return error.response; // bubble it up (with status)
  ...
  async get_scan(scanPath) {
    ...
    return this.post({ url: url, responseType: 'stream' }).then(response => {
      return response;
    });
  }
   ...
   * @memberof doxieNode
   */
  async get_thumbnail(scanPath, retryCount = 3, retryDelay = 1000) {
